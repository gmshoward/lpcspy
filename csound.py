...
class Dynamics(object):
    ...
    def average_level(self):
        ...
        for dp in self.envelope[1:]:
            ...
            level = level + (segment_average * dp_prev.duration)
            dp_prev = dp
...
"""
class Note(Event):
    def __init__(self, start=0, duration=0, dynamics=Dynamics(), articulation=None, pitch=None):
        self.pitch = pitch
        super(Note, self).__init__(start, duration, dynamics, articulation)

        

    def emit(self, instr, start=0, dynamics=Dynamics(), articulation=None, portamento=None):
        if (self.articulation == None):
            passed_articulation = articulation
        else:
            passed_articulation = self.articulation
